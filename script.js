...
const refreshField = (shuffled = false) => {
  ...
  document.getElementById('game-title-9').style.display = 'none';
  document.getElementById('game-title-15').style.display = 'none';

  if (currentGame === 15) {
    document.getElementById('game-title-15').style.display = 'block';
  } else {
    document.getElementById('game-title-9').style.display = 'block';
  }
...
const restartGame = () => {
  if (isPuzzleSolved()) {
    ...
    document.querySelector('.heading').innerHTML = 'You win!';
...
const handleKey = (event) => {
  ...
  if (pressedKey === 'KeyR') {
    resetGame(); // Call the resetGame function for retry on 'R' press
    return;
  }
...
const resetGame = () => {
  ...
  document.querySelector('.subheading').textContent = 'Time: 0 seconds';
  
  
  shuffleGridWithLastSteps();

  
  startTimer(); 
};

const shuffleGridWithLastSteps = () => {
  const tableEl = document.querySelector('table');
  animateShuffle(tableEl, lastShuffleSteps); 
};

document.getElementById('retry-game-button').addEventListener('click', resetGame);
